...
- A CLI with an unparseable version is treated conservatively and receives the four-Hook compatibility configuration.
...
| CLI below `0.147.0` | Ask to upgrade to `@openai/codex@0.147.0` | Consent enables full mode; refusal installs compatibility mode |
| CLI at or above `0.147.0` | Keep the installed CLI | Full mode |
| Version unknown | Do not modify the global CLI | Compatibility mode |
...
- If an upgrade fails for an existing old CLI, VibeSignal keeps that CLI and installs compatibility mode.
- If `codex --version` fails or cannot be parsed, VibeSignal does not mutate the global CLI and installs compatibility mode.
